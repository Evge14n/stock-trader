...
async def query_with_model(
    ...
    cache_key = hashlib.md5(f"{model}:{system}:{prompt}:{temperature}".encode()).hexdigest()
...
async def query_ensemble(
    ...
    tasks = [query_with_model(prompt, m, system, temperature, max_tokens) for m in models]
    responses = await asyncio.gather(*tasks)
    return list(zip(models, responses, strict=False))
